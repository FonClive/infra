...
data "aws_iam_policy_document" "coredns_assume_role_policy" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
...
resource "aws_iam_role" "coredns" {
  name               = "${var.cluster_name}-coredns-role"
  assume_role_policy = data.aws_iam_policy_document.coredns_assume_role_policy.json
}

data "aws_iam_policy_document" "kube_proxy_assume_role_policy" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
